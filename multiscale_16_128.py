...
#split validation
train_input_x = concatenated_input[:train_size] 
val_input_x= concatenated_input[train_size:train_size+val_size]
test_input_x = concatenated_input[train_size+val_size:]  

# Split input
train_input_y = concatenated_perm[:train_size] 
val_input_y= concatenated_perm[train_size:train_size+val_size]
test_input_y = concatenated_perm[train_size+val_size:]  

# Split target
train_target = concatenated_target[:train_size]  
val_target= concatenated_target[train_size:train_size+val_size]
test_target = concatenated_target[train_size+val_size:]  
...
for train_input_x, train_input_y, train_target in train_loader:
    print(f"Train Input coarse sat Shape: {train_input_x.shape}")
    print(f"Train Input Perm Shape: {train_input_y.shape}")
    print(f"Train Target Shape: {train_target.shape}")
    break  # Print for only one batch

for val_input_x, val_input_y, val_target in val_loader:
    print(f"Val Input coarse sat Shape: {val_input_x.shape}")
    print(f"Val Input Perm Shape: {val_input_y.shape}")
    print(f"Val Target Shape: {val_target.shape}")
    break

for test_input_x, test_input_y, test_target in test_loader:
    print(f"Test Input coarse sat Shape: {test_input_x.shape}")
    print(f"Test Input Perm Shape: {test_input_y.shape}")
...
class FSRCNN3D(nn.Module):
    def __init__(self, d, p, m, input_channels=1, upscaling_factor_xy=2, upscaling_factor_z=2):
        ...
        self.deconv = nn.ConvTranspose3d(d, 1, kernel_size=(4,8,8), 
...
model = FSRCNN3D(d=32, m=4,p=16, input_channels=1, upscaling_factor_xy=8, upscaling_factor_z=6)
model.to(device)  
...
model_path = os.path.join(save_dir, "multiscale_16_128.pth")
